...
class StructuredPDFInfo
{
      ...
      throws IOException
   {
      ...
      final COSBase   pagemode               = root.getDictionaryObject(COSName.PAGE_MODE);

      if (pagemode instanceof COSName)
      {
         if (((COSName)pagemode).getName().equalsIgnoreCase("/UseOutlines"))
         {
            if (root.getDictionaryObject(COSName.OUTLINES) != null)
            {
      ...
      traverseElements(open_document_keys, lc_root, lc_outlines, lc_open_document, lc_first_page_private, lc_first_page_shared, lc_other_page_private,
                       lc_other_page_shared, lc_thumbnail_private, lc_thumbnail_shared, lc_other);
   ...
   private void initData(final ArrayList<COSBase> lc_root,
                         final List<COSBase>      lc_open_document)
      throws ArithmeticException
   {
      ...
      lc_open_document.forEach((obj) -> {part4.add(obj);});
   }


   private void gatherSharedHintTable()
      throws ArithmeticException
   {
   ...
   private void processPart9(final List<COSBase> lc_other,
                             final List<COSBase> lc_thumbnail_private,
                             final List<COSBase> lc_thumbnail_shared,
                             final boolean       outlines_in_first_page,
                             final List<COSBase> lc_outlines)
      throws ArithmeticException
   {
      ...
      {
         final COSBase thumb = pages.get(i).getDictionaryObject(COSName.THUMB);
      ...
      lc_thumbnail_shared.forEach((iterthumshar) -> {part9.add(iterthumshar);});
      ...
      lc_other.forEach((iterother) -> {part9.add(iterother);});
      ...
      {
         throw new ArithmeticException("calculateLinearizationData: wrong number of objects placed (num_placed = " + num_placed
                                       + "; number of objects: " + num_wanted);
      }
   }
   ...
   private void processPart8(final List<COSBase> lc_other_page_shared)
   {
      ...
      lc_other_page_shared.forEach((itershared) -> {part8.add(itershared);});
   }


   private void processPart7(final Set<COSBase> lc_other_page_private)
      throws ArithmeticException
   {
   ...
   private void processPart6(final List<COSBase> lc_first_page_private,
                             final List<COSBase> lc_first_page_shared,
                             final boolean       outlines_in_first_page,
                             final List<COSBase> lc_outlines)
      throws ArithmeticException
   {
      ...
      // hint tables.
      lc_first_page_private.forEach((page) -> {part6.add(page);});

      lc_first_page_shared.forEach((page) -> {part6.add(page);});
   ...
   private void traverseElements(final List<COSName>      open_document_keys,
                                 final ArrayList<COSBase> lc_root,
                                 final List<COSBase>      lc_outlines,
                                 final List<COSBase>      lc_open_document,
                                 final List<COSBase>      lc_first_page_private,
                                 final List<COSBase>      lc_first_page_shared,
                                 final Set<COSBase>       lc_other_page_private,
                                 final List<COSBase>      lc_other_page_shared,
                                 final List<COSBase>      lc_thumbnail_private,
                                 final List<COSBase>      lc_thumbnail_shared,
                                 final List<COSBase>      lc_other)
      throws ArithmeticException
   {
   ...
   private void pushOutlinesToPart(final List<COSBase> part,
                                   final List<COSBase> lc_outlines)
   {
      final COSObject root     = (COSObject)document.getTrailer().getItem(COSName.ROOT);
      final COSObject outlines = (COSObject)root.getItem(COSName.OUTLINES);
