   ...
   implements Closeable
{
    ...
    */
   private int integrateDocument(final COSDocument doc)
      throws IOException
   {
      ...
      final COSDictionary pages      = (COSDictionary)root.getDictionaryObject(COSName.PAGES);
      ...
      final PageIterator  iter       = new PageIterator((COSObject)docRoot.getItem(COSName.PAGES));
      int                 pagesCount = 0;
   ...
   private static COSObject createPagesObjekt()
      throws IOException
   {
      ...
      final COSObject retVal = new COSObject(pagesDict);

      retVal.setObjectNumber(DUMMY_MAX_VALUE_LONG + 1);
      return retVal;
   }


   private COSObject createDocumentRoot()
      throws IOException
   {
      ...
      final COSObject retVal = new COSObject(rootDict);

      retVal.setObjectNumber(DUMMY_MAX_VALUE_LONG + 0);
      return retVal;
   }


   private COSDictionary createDocumentTrailer()
      throws IOException
   {
    ...
    *
    * @return The trailer passed with the added ID.
    */
   COSDictionary addIDToTrailer(final COSDictionary trailer)
   {
      ...
      md5.update((Long.toString(System.currentTimeMillis()) + "PDF_BOX_SALT_STRING" + this.hashCode()).getBytes(Charsets.ISO_8859_1));

      final COSDictionary info = (COSDictionary)trailer.getDictionaryObject(COSName.INFO);

      if (info != null)
      {
         info.getValues().forEach((cosBase) -> {md5.update(cosBase.toString().getBytes(Charsets.ISO_8859_1));});
      }
    ...
    * @version $Id$
    */
   private static final class PageIterator
