...
public class Linearizer
{
      ...
      throws IOException
   {
      ...
      // First half objects
      queue.enqueueDummy(PDFDummyObjects.LINDICT);
      queue.enqueueDummy(PDFDummyObjects.FIRSTXREF);
      queue.enqueuePart(info.part4);
      queue.enqueueDummy(PDFDummyObjects.HINTSTREAM);
      ...
      queue.enqueueDummy(PDFDummyObjects.SECONDXREF);
      offsets.secondTrailerSize = queue.getNextID();

      try
      {
         final COSObject linearizedDictObject = this.createLinearizedDictionary(queue.get(PDFDummyObjects.LINDICT).objNumber, writer);
         ...
         offsets.hintLength = writtenStore.add(SpecialParts.HINTSTREAM, writer.getAndResetParts());

         // we have all mandatory sizes, compute missed values
         ((COSArray)linearizedDictObject.getDictionaryObject(COSName.H)).set(0, COSInteger.get(offsets.linDictOffset));
         ((COSArray)linearizedDictObject.getDictionaryObject(COSName.H)).set(1, COSInteger.get(offsets.hintLength));

         final List<COSWriterXRefEntry>      xrefentries       = writer.retrieveXRefEntries();
         final List<COSWriterXRefEntry>      xrefentries_first = new ArrayList<>();
         final ArrayList<COSWriterXRefEntry> xrefentries_sec   = new ArrayList<>(queue.size());
         long                                cumulateLength    = this.fillXRefEntries(xrefentries, xrefentries_first, xrefentries_sec, writtenStore,
                                                                                      queue, info.getPart4EndMarker(), info.getPart6EndMarker());
   ...
   private COSObject fillHintStream(final StructuredPDFInfo        info,
                                    final LinearizationInformation offsets,
                                    final PDFObjectQueue           queue,
                                    final LinearizedPDFWriter      writer)
      throws IOException
   {
      ...
      final COSObject     hintStreamObject  = this.createHintStream(queue.get(PDFDummyObjects.HINTSTREAM).objNumber, writer);
   ...
   private void setObjectNumbers(final PDFObjectQueue      queue,
                                 final LinearizedPDFWriter writer)
   {
   ...
   private long fillXRefEntries(final List<COSWriterXRefEntry> xrefentries,
                                final List<COSWriterXRefEntry> xrefentries_first,
                                final List<COSWriterXRefEntry> xrefentries_sec,
                                final WrittenObjectStore       objParts,
                                final PDFObjectQueue           queue,
                                final COSBase                  part4_end_marker,
                                final COSBase                  part6_end_marker)
   {
      xrefentries.sort(LinearizedPDFWriter.XREFCOMP);

      List<COSWriterXRefEntry> xrefentries_write = xrefentries_first;

      xrefentries_first.add(getObjectXrefEntryBinary(xrefentries, queue.get(PDFDummyObjects.LINDICT).objNumber));

      long cumulateLength = objParts.getLength(SpecialParts.PARTS_BEFORE_FIRST_XREF);

      for (final HashMap.Entry<COSBase, ObjectMetaData> entry : queue.entrySet())
      {
         final COSBase        obj         = entry.getKey();
         final ObjectMetaData objData     = entry.getValue();

         long                 beginObject = cumulateLength;

         cumulateLength += objData.objLength;

         COSWriterXRefEntry xrefentry = getObjectXrefEntryBinary(xrefentries, objData.objNumber);

         xrefentry.setOffset(beginObject);
         xrefentries_write.add(xrefentry);

         if (part4_end_marker == obj)
         {
            beginObject    = cumulateLength;
            cumulateLength += objParts.getLength(SpecialParts.HINTSTREAM);
            xrefentry      = getObjectXrefEntryBinary(xrefentries, queue.get(PDFDummyObjects.HINTSTREAM).objNumber);
            xrefentry.setOffset(beginObject);
            xrefentries_write.add(xrefentry);
         }
   ...
   private void writeObjects(final PDFObjectQueue           queue,
                             final LinearizedPDFWriter      writer,
                             final WrittenObjectStore       objParts,
                             final LinearizationInformation offsets,
                             final StructuredPDFInfo        info)
      throws IOException
   {
      long          outlineCount   = 0;
      ...
      for (final HashMap.Entry<COSBase, ObjectMetaData> entry : queue.entrySet())
      {
         ...
         {
            objParts.add(SpecialParts.FIRST_PART_OBJECTS, writer.getAndResetParts());
      ...
      objParts.add(SpecialParts.SECOND_PART_OBJECTS, writer.getAndResetParts());
   }


   private COSWriterXRefEntry getObjectXrefEntryBinary(final List<COSWriterXRefEntry> xrefentries,
                                                       final long                     objNr)
   {
      int start = 0;
      ...
      while (start <= end)
      {
         ...
         final COSWriterXRefEntry midEntry = xrefentries.get(mid);
         ...
         final long midNum = midEntry.getKey().getNumber();
   ...
   private COSObject createLinearizedDictionary(final int                 lindict_id,
                                                final LinearizedPDFWriter writer)
      throws IOException
   {
      final COSObject linearizedDictionary = createDummyLinearizedDictionary();

      linearizedDictionary.setObjectNumber(lindict_id);
   ...
   private COSObject createHintStream(final int                 hint_id,
                                      final LinearizedPDFWriter writer)
      throws IOException
   {
      ...
      hintStreamObject.setObjectNumber(hint_id);
   ...
   private COSObject createDummyLinearizedDictionary()
      throws IOException
   {
      final long          DUMMY_MAX_VALUE_LONG = 1111111111111111111L;
      final int           DUMMY_MAX_VALUE_INT  = 1111111111;
      ...
      retVal.setObjectNumber(DUMMY_MAX_VALUE_LONG);
      return retVal;
   }
