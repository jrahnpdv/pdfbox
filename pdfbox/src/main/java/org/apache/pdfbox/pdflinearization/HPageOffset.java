...
import static org.apache.pdfbox.pdflinearization.IO.nbits;
...
class HPageOffset
{
    ...
    */
   static HPageOffset filledPageOffsetHints(final StructuredPDFInfo info,
                                            final PDFObjectQueue    queue,
                                            final long              lin_dict_offset,
                                            final int               pageCount)
   {
      ...
      pageOffsetHints.nbits_delta_nobjects       = nbits(max_nobjects - min_nobjects);
      pageOffsetHints.min_page_length            = (int)min_length;
      pageOffsetHints.nbits_delta_page_length    = nbits((int)max_length - (int)min_length);
      pageOffsetHints.nbits_nshared_objects      = nbits(max_shared);
      pageOffsetHints.nbits_shared_identifier    = nbits(info.sharedObjectData.nshared_total);
    ...
    */
   void writeHPageOffset(final BitWriter w)
      throws IOException
   {
      ...
      for (final HPageOffset.HPageOffsetEntry en : entries)
      {
